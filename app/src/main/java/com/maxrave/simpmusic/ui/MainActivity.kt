...
import com.maxrave.simpmusic.ads.admob.AdMobInterstitialAdSearch
...
import com.maxrave.simpmusic.downloader.util.DialogUtils
...
import com.maxrave.simpmusic.ui.widget.AppTerminationService
import com.maxrave.simpmusic.ui.widget.BasicWidget
import com.maxrave.simpmusic.utils.NotificationReceiver
    ...
    override fun onRequestPermissionsResult(
        ...
        for (permission in grantResults) {
            if (permission == PackageManager.PERMISSION_DENIED) {
                if (requestCode == PermissionHelper.DOWNLOADS_REQUEST_CODE || requestCode == PermissionHelper.DOWNLOAD_DIALOG_REQUEST_CODE) {
                    DialogUtils.show(
                        this,
        ...
        when (requestCode) {
            PermissionHelper.DOWNLOADS_REQUEST_CODE -> startActivity(
                Intent(
                    this,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        googleMobileAdsConsentManager.gatherConsent(this) {
            if (googleMobileAdsConsentManager.canRequestAds) {
                AdMobInterstitialAdSearch.getInstance().init(this)
                AdMobInterstitialAdDownload.getInstance().init(this)
            }
...
//        // Check if the migration has already been done or not
//        if (getString(FIRST_TIME_MIGRATION) != STATUS_DONE) {
//            if (SUPPORTED_LANGUAGE.codes.contains(LanguagePreference.getSelectedLanguage(this))) {
//                putString(SELECTED_LANGUAGE, LanguagePreference.getSelectedLanguage(this))
//                if (SUPPORTED_LOCATION.items.contains(LanguagePreference.getSelectedLanguage(this).toUpperCase())) {
//                    putString("location", LanguagePreference.getSelectedLanguage(this).toUpperCase())
//                } else {
//                    putString("location", "US")
//                }
//                YouTube.locale = YouTubeLocale(
//                    gl = getString("location") ?: "US",
//                    hl =LanguagePreference.getSelectedLanguage(this)
//                )
//            } else {
//                putString(SELECTED_LANGUAGE, "en-US")
//                YouTube.locale = YouTubeLocale(
...
//            }
//        }
//        if (LanguagePreference.getSelectedLanguage(this) != getString(
//                SELECTED_LANGUAGE
//            )
//        ) {
//
...
//        }
        ...
        navController.addOnDestinationChangedListener { nav, destination, _ ->
            when (destination.id) {
                R.id.bottom_navigation_item_home, R.id.mainSettingsFragment, R.id.recentlySongsFragment, R.id.moodFragment -> {
        ...
        enableEdgeToEdge()
        startService(Intent(this, AppTerminationService::class.java))
        AdMobInterstitialAdSong.getInstance().init(this)
        AdMobInterstitialAdArtist.getInstance().init(this)
        ...
        valueEventListener = object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                for (child in dataSnapshot.children) {
                    ...
                    if (adConfig != null) {
                        SharedPrefsHelper.saveObjectPrefs(this@MainActivity, child.key, adConfig)
                    }
                }
            }
    ...
    private fun cleanAdConfigValueListener() {
    ...
    override fun onStop() {
        super.onStop()
        cleanAdConfigValueListener()
    }


    private fun requestToPinWidget() {
        val prefApp: SharedPreferences = getSharedPreferences("prefApp", Context.MODE_PRIVATE)
        val editor: SharedPreferences.Editor = prefApp.edit()
        val isFirstTime = prefApp.getBoolean("isFirstTime", true)
        if (isFirstTime) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                val appWidgetManager = getSystemService(AppWidgetManager::class.java)
                val myProvider = ComponentName(this, BasicWidget::class.java)
    ...
    private fun scheduleDailyNotifications() {
        ...
        val intent10AM = Intent(this, NotificationReceiver::class.java)
        val pendingIntent10AM = PendingIntent.getBroadcast(this, 0, intent10AM, PendingIntent.FLAG_IMMUTABLE)

        val calendar10AM = Calendar.getInstance().apply {
        ...
        if (calendar10AM.timeInMillis < System.currentTimeMillis()) {
            calendar10AM.add(Calendar.DAY_OF_YEAR, 1)
        }

        alarmManager.setRepeating(
            AlarmManager.RTC_WAKEUP,
            calendar10AM.timeInMillis,
            AlarmManager.INTERVAL_DAY,
            pendingIntent10AM
        )

        // Schedule 10 PM notification
        val intent10PM = Intent(this, NotificationReceiver::class.java)
        val pendingIntent10PM = PendingIntent.getBroadcast(this, 1, intent10PM, PendingIntent.FLAG_IMMUTABLE)

        val calendar10PM = Calendar.getInstance().apply {
        ...
        // If the time is already past for today, schedule for tomorrow
        if (calendar10PM.timeInMillis < System.currentTimeMillis()) {
            calendar10PM.add(Calendar.DAY_OF_YEAR, 1)
        }

        alarmManager.setRepeating(
            AlarmManager.RTC_WAKEUP,
            calendar10PM.timeInMillis,
            AlarmManager.INTERVAL_DAY,
            pendingIntent10PM
        )
    }
